...
Overview
----------------------------------------------------------------------------
1. Slide through the time series row-by-row:
...
2. Persist fused embeddings, OHLCV, and timestamp to:
...
import os, logging, torch, numpy as np, pandas as pd, json
...
from ..lib.utils import to_python_types
...
class NeuralFusionCore_infer:
    def __init__(self, MODEL_CHECKPOINT: str = "data/outputs/model_weights.pt",mongo_collection=None):
        """
        Initialize NeuralFusionCore inference class.

        Args:
            MODEL_CHECKPOINT (str): Path to model weights.
            mongo_collection: A MongoDB collection instance.
        """
        self.mongo_col = mongo_collection
        self.MODEL_CHECKPOINT = MODEL_CHECKPOINT

    def load_model(self, configs, feat_cols_len, stock_list_len, count_dim, device='cpu'):
        ...
        weights_path = getattr(P, "weights_pt", self.MODEL_CHECKPOINT)
    ...
    # --------------------------- Save predictions ---------------------------
    def save_fused_embedding_predictions(self, date, fused, df_row, stock_list):
        records = []
        ...
        self.mongo_col.insert_many(clean_records)
        logging.info("Saved bridge results to MongoDB")


    # --------------------------- Inference with sliding window ---------------------------
    def run_inference(self, df_not_norm_te, df_te, feat_cols, data_stamp_cols, stock_list, cnt_cols, device='cpu'):
        ...
        model = self.load_model(configs, len(feat_cols), len(stock_list), count_dim, device)
        ...
        for idx in range(num_rows):
            ...
            self.save_fused_embedding_predictions(df_te.iloc[idx][date_col], fused, df_not_norm_te.iloc[idx], stock_list)

            if (idx + 1) % 50 == 0:
                print(f"Processed {idx + 1}/{num_rows} rows")

    def FusedEmbedding(self, mongo_collection, model_checkpoint,device: str = "CPU"):
        ...
        self.run_inference(df_te_not_norm, df_te, feat_cols, data_stamp_cols, stock_list, cnt_cols, device=device)